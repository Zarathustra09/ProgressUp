...
class StaffParentController extends Controller
{
    //
    public function index()
    {
        $branchId = auth()->user()->branch_id;
        $users = User::where('role_id', 0)->where('branch_id', $branchId)->get();
        return view('staffPages.parent.index', compact('users'));
    }
    ...
    public function store(Request $request)
    {
        ...
        User::create([
            ...
            'branch_id' => auth()->user()->branch_id, // Use the currently logged-in user's branch_id
        ]);

        return redirect()->route('staffParent.index')->with('success', 'User created successfully.');
    }
